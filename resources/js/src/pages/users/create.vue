<template>
  <DefaultLayout :access="access">
    <div class="">
      <div class="">
        <PageTitle :route="`${route_back}/users`" title="Users Form" />
      </div>

      <ValidationObserver v-slot="{ handleSubmit, pristine }" ref="validation_observer">
        <form @submit.prevent="handleSubmit(submitForm)">
          <v-container>
            <v-card>
              <v-card-title primary-title> Information </v-card-title>
              <v-card-text>
                  ...
                  <div>
                    ...
                    >
                      <v-select
                        ...
                        :item-value="(item) => item.PK_section_ID"
                        v-model="form.section_id"
                        prepend-icon="mdi-clipboard-account"
                        outlined
                        filled
                        label="Select Unit Section"
                        :error-messages="errors[0] ? errors[0] : form.errors.section_id"
                        clearable
                      ></v-select>
                    </ValidationProvider>
                  </div>
              ...
              <v-card-text>
                <v-card flat>
                  <v-card-title primary-title>
                    Select User Role
                    <v-spacer></v-spacer>
                    <v-btn dark @click="roles_modal.active = true">Add Role</v-btn>
                  </v-card-title>
                  <v-card-text>
                    <v-select
                    ...
                    <div class="grid grid-cols-2 gap-2">
                      <div v-for="role in access_role" :key="role.id">
                        <v-checkbox
