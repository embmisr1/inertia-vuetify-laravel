    ...
    >
      <template #top-left>
        <b-field type="is-dark">
          <b-select placeholder="Rows Per Page" v-model="filter.per_page" size="is-small">
    ...
    <v-dialog v-model="dialog" max-width="500" persistent>
      <v-card>
        ...
        <v-card-text>
          <div class="space-y-4">
            <div>
              <b-field label="Province" label-position="inside" type="is-dark">
                <b-select v-model="prov_filter" placeholder="Select a Province" expanded>
                  <option
                    v-for="option in province_list"
                    :value="option.PK_province_ID"
                    :key="option.PK_province_ID"
            ...
            <div>
              <b-field label="Municipality" label-position="inside" type="is-dark">
                <b-select
            ...
            <div>
              <b-field label="Barangay" label-position="inside" type="is-dark">
                <b-select
                  v-model="brgy_filter"
                  placeholder="Select a Barangay"
                  expanded
          ...
          >
            ...
            <v-checkbox
              v-if="
                selectedSearchCategory == 'MONITORING' || selectedSearchCategory == 'NOV'
              "
        ...
        <v-card-actions>
          ...
          <v-btn color="red darken-1" class="white--text" @click="clearFilter"
            ><v-icon> mdi-autorenew </v-icon>
            Clear Filter
          </v-btn>
          <v-btn color="blue darken-1" class="white--text" @click="filterUniverse"
            ><v-icon> mdi-filter </v-icon>
            Filter
          </v-btn>
          <v-btn color="green darken-1" class="white--text" @click="exportUniverse"
  ...
  methods: {
    ...
    async provinceDropdown(val) {
      try {
        ...
        const municipality = await axios.get(`${urls.provinceDropdown}/${val}`);
    ...
    async municipalityDropdown(val) {
      try {
        this.loading = true;
        const barangay = await axios.get(`${urls.municipalityDropdown}/${val}`);
