    ...
    <v-card class="p-4" elevation="0">
      <div v-if="form_permit_info.addFileForm">
        <div class="grid grid-cols-5 gap-y-0 gap-x-4 ml-8">
          ...
          <div>
            <ValidationProvider vid="law" name="Law" rules="required" v-slot="{ errors }">
              <v-autocomplete
        ...
        <div class="text-center">
          <v-btn
            v-if="has_permit || (has_hazwaste && form_permit_info.perm_law === 'RA 6969')"
            depressed
            color="primary"
            type="submit"
          >
          ...
          <v-btn depressed color="error" type="button" @click="resetPermit">
          ...
          >
            <template v-slot:item.perm_file="{ item }">
              <a
                ...
                :href="`/app/attachments?type=permits-ftp&id=${item.id}`"
                target="_blank"
              >
            ...
            <template v-slot:item.actions="{ item }">
              <div v-if="has_permit || (has_hazwaste && item.perm_law === 'RA 6969')">
                <v-icon small class="mr-2" @click="editPermit(item)"> mdi-pencil </v-icon>
                <v-icon small @click="deletePermit(item)"> mdi-delete </v-icon>
              </div>
              <v-icon v-else small class="mr-2" @click="editPermit(item)">
                mdi-eye
              </v-icon>
            </template>
          </v-data-table>
