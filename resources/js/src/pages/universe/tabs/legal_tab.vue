  ...
  <v-card class="p-4" elevation="0">
    <div v-if="form_legal_info.addFileForm">
      ...
      <div class="text-center">
        ...
        <v-btn depressed color="error" type="button" @click="resetLegal">
        ...
        >
          <template v-slot:item.nov_file="{ item }">
            <a
              ...
              :href="`/app/attachments?type=legal-ftp&id=${item.id}`"
              target="_blank"
            >
          ...
          <template v-slot:item.actions="{ item }" v-if="has_permit">
            <div v-if="has_permit">
              <v-icon small class="mr-2" @click="editLegal(item)"> mdi-pencil </v-icon>
              <v-icon small @click="deleteLegal(item)"> mdi-delete </v-icon>
            </div>
            <v-icon v-else small class="mr-2" @click="editLegal(item)"> mdi-eye </v-icon>
          </template>
        </v-data-table>
      </v-card>
    </template>
